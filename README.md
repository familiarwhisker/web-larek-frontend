...
### Принципы валидации
- **Валидация в Model слое** - бизнес-логика валидации находится в модели
- **Реальное время** - валидация происходит при каждом изменении поля
- **UI обновление** - View обновляется на основе результатов валидации
- **Типизация** - все данные валидации строго типизированы

### Принципы работы с формами
- **Инициализация в конструкторе** - DOM элементы и события привязываются в конструкторе
- **Автоматическая очистка** - формы очищаются при повторном открытии
- **Состояние кнопок** - кнопки отправки блокируются при некорректных данных
- **Обработка ошибок** - ошибки валидации отображаются пользователю
